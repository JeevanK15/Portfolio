...
export async function POST(request: Request) {
  try {
    ...
    const key = process.env.RESEND_API_KEY; const recipient = process.env.CONTACT_EMAIL;
    if (!key || !recipient) return NextResponse.json({ error: "Contact service is not configured." }, { status: 503 });
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: "Portfolio contact <onboarding@resend.dev>", to: [recipient], reply_to: email, subject: `Portfolio message from ${name}`, text: `Name: ${name}\nEmail: ${email}\n\n${message}` }), signal: AbortSignal.timeout(10000) });
